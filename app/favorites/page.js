    ...
    <div className="min-h-screen bg-gradient-to-b from-black to-zinc-900 text-white">
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-white">My Favorite Designs</h1>
          
          <div className="flex flex-wrap gap-3">
            <Button
              onClick={() => router.push('/dashboard')}
              className="bg-zinc-800 hover:bg-zinc-700 text-white transition-colors duration-300 flex items-center gap-2 flex-1 sm:flex-none justify-center"
            >
            ...
            <Button
              ...
              className="bg-gradient-to-r from-[#1e3a5c] via-[#22d3ee] to-[#4ade80] text-white hover:opacity-90 transition-all duration-300 flex-1 sm:flex-none justify-center"
            >
              Create New Design
            </Button>
          </div>
            ...
            {favorites.map((favorite) => (
              <div key={favorite.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-xl transition-all duration-300 hover:shadow-[0_0_15px_rgba(34,211,238,0.3)] hover:border-[#22d3ee]/50">
                <div 
                  ...
                  className="relative cursor-pointer h-40 sm:h-48 overflow-hidden"
                ...
                >
                  <Image 
                    ...
                    priority={favorite.id === favorites[0]?.id}
                    style={{ objectFit: 'cover' }}
                    onContextMenu={(e) => e.preventDefault()}
                  />
                  {/* Add DecorMind watermark overlay */}
                  <div className="absolute top-2 right-2 text-sm font-bold p-[2px_6px] rounded-md bg-teal-400 text-white drop-shadow-[0_1px_1px_rgba(0,0,0,0.8)]">
                    DecorMind
                  </div>
                ...
                <div key={`content-${favorite.id}`} className="p-3 sm:p-4">
                  <div key={`header-${favorite.id}`} className="flex justify-between items-start mb-2 sm:mb-3">
                    <div key={`info-${favorite.id}`} className="mr-2">
                      <h3 className="text-base sm:text-lg font-medium text-white truncate">{favorite.roomType} Design</h3>
                      <p className="text-zinc-400 text-xs sm:text-sm">{favorite.style} Style</p>
                    </div>
                    <div className="flex gap-1 sm:gap-2 flex-shrink-0">
                      <button 
                        ...
                        className="bg-zinc-800 hover:bg-zinc-700 text-white p-1.5 sm:p-2 rounded-full transition-colors duration-300 group relative"
                        title="Share design"
                      ...
                      >
                        ...
                        <span className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-black text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap pointer-events-none z-10">
                          Share Design
                        </span>
                      </button>
                      <button 
                        ...
                        className="bg-pink-600 hover:bg-pink-700 text-white p-1.5 sm:p-2 rounded-full transition-colors duration-300 group relative"
                        title="Remove from favorites"
                        aria-label="Remove from favorites"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" fill="currentColor" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                        </svg>
                        <span className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-black text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap pointer-events-none z-10">
                  ...
                  <p key={`date-${favorite.id}`} className="text-zinc-500 text-xs mb-3">
                    Saved on {new Date(favorite.date).toLocaleDateString()}
                  </p>
                  
                  <div key={`actions-${favorite.id}`} className="flex justify-between gap-2">
                    <Button
                      key={`view-btn-${favorite.id}`}
                      onClick={() => handleViewDesign(favorite)}
                      className="bg-zinc-800 hover:bg-zinc-700 text-white transition-colors duration-300 text-xs sm:text-sm px-2 sm:px-3 py-1 h-auto flex-1"
                    >
                      View Design
                    </Button>
                    ...
                    <Button
                      ...
                      onClick={() => router.push('/redesign')}
                      className="bg-[#22d3ee] text-black hover:bg-[#22d3ee]/90 transition-all duration-300 text-xs sm:text-sm px-2 sm:px-3 py-1 h-auto flex-1"
                    >
                      Create Similar
                    </Button>
                  </div>
      ...
      {showModal && selectedDesign && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden w-full max-w-4xl max-h-[95vh] sm:max-h-[90vh] flex flex-col">
            <div className="flex justify-between items-center p-3 sm:p-4 border-b border-zinc-800">
              <h3 className="text-lg sm:text-xl font-bold text-white truncate">{selectedDesign.roomType} Design</h3>
              <button 
            ...
            <div className="overflow-y-auto flex-grow">
              <div className="relative">
                <div className="relative w-full h-[40vh] sm:h-[50vh] md:h-[60vh] lg:h-[70vh]">
                  ...
                  />
                  {/* Add DecorMind watermark overlay */}
                  <div className="absolute top-2 right-2 sm:top-4 sm:right-4 text-xs sm:text-sm font-bold p-[2px_6px] rounded-md bg-teal-400 text-white drop-shadow-[0_1px_1px_rgba(0,0,0,0.8)]">
                    DecorMind
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 sm:p-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
                  <div>
                    <h4 className="text-zinc-400 text-xs sm:text-sm mb-1">Style</h4>
                    <p className="text-white text-base sm:text-lg">{selectedDesign.style}</p>
                  </div>
                  <div>
                    <h4 className="text-zinc-400 text-xs sm:text-sm mb-1">Saved On</h4>
                    <p className="text-white text-base">{new Date(selectedDesign.date).toLocaleDateString()}</p>
                  </div>
                </div>
                ...
                {showShareOptions && (
                  <div ref={shareOptionsRef} className="bg-zinc-800 p-3 rounded-lg mb-4 w-full">
                    <div className="flex justify-between items-center mb-2">
                      <p className="text-zinc-300 text-xs sm:text-sm">Share via:</p>
                      <button 
                    ...
                    <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2">
                      <button 
                        ...
                        className="bg-[#25D366] hover:bg-opacity-90 text-white p-1.5 sm:p-2 rounded-md transition-colors duration-300 flex items-center justify-center sm:justify-start gap-1 text-xs sm:text-sm"
                      ...
                      <button 
                        ...
                        className="bg-[#1877F2] hover:bg-opacity-90 text-white p-1.5 sm:p-2 rounded-md transition-colors duration-300 flex items-center justify-center sm:justify-start gap-1 text-xs sm:text-sm"
                      ...
                      <button 
                        ...
                        className="bg-[#1DA1F2] hover:bg-opacity-90 text-white p-1.5 sm:p-2 rounded-md transition-colors duration-300 flex items-center justify-center sm:justify-start gap-1 text-xs sm:text-sm"
                      ...
                      <button 
                        ...
                        className="bg-zinc-600 hover:bg-zinc-500 text-white p-1.5 sm:p-2 rounded-md transition-colors duration-300 flex items-center justify-center sm:justify-start gap-1 text-xs sm:text-sm"
                ...
                <div className="flex flex-wrap sm:flex-nowrap gap-2 sm:gap-3 mt-4 sm:mt-6">
                  <Button
                    ...
                    className="bg-zinc-800 hover:bg-zinc-700 text-white transition-colors duration-300 flex items-center justify-center gap-1 sm:gap-2 text-xs sm:text-sm py-2 flex-1"
                  >
                  ...
                  <Button
                    ...
                    className="bg-zinc-700 hover:bg-zinc-600 text-white transition-colors duration-300 flex items-center justify-center gap-1 sm:gap-2 text-xs sm:text-sm py-2 flex-1"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 sm:h-4 sm:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
                    </svg>
                  ...
                  <Button
                    ...
                    className="bg-[#22d3ee] text-black hover:bg-[#22d3ee]/90 transition-all duration-300 text-xs sm:text-sm py-2 flex-1"
                  >
                    Create Similar
                  </Button>
                </div>
      ...
      {/* Mobile action buttons - fixed to bottom on small screens */}
      {favorites.length > 0 && !showModal && (
        <div className="md:hidden fixed bottom-4 right-4 z-50 flex flex-col gap-2">
          <button
            onClick={() => router.push('/redesign')}
            className="bg-[#22d3ee] text-white p-2.5 rounded-full shadow-lg hover:opacity-90 transition-all duration-300"
          ...
          <button
            ...
            className="bg-gradient-to-r from-[#1e3a5c] via-[#22d3ee] to-[#4ade80] text-white p-2.5 rounded-full shadow-lg hover:opacity-90 transition-all duration-300"
            aria-label="Share design"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
