...
function SimplifiedPricingComponent() {
  ...
  const renderActionButton = (plan, isCurrent, isDisabled) => {
    if (typeof window === 'undefined') {
      ...
      return (
        <button className="mt-6 w-full bg-[#C5A790] hover:bg-[#b0927a] text-black py-2 rounded-lg">
          Loading...
        </button>
      );
    }
      ...
      if (!isSignedIn) {
        ...
        return (
          <PaymentButton
            ...
            className={`mt-6 w-full ${isDisabled ? "bg-gray-500" : "bg-[#C5A790] hover:bg-[#b0927a]"} text-black py-2 rounded-lg`}
            onSuccess={(response) => handlePaymentSuccess(plan.planKey, response)}
          />
        );
      }
