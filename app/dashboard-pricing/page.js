...
function SimplifiedPricingComponent() {
  ...
  const renderActionButton = (plan, isCurrent, isDisabled) => {
    if (typeof window === 'undefined') {
      ...
      return (
        <button className="mt-6 w-full bg-cyan-400 hover:bg-cyan-500 text-black py-2 rounded-lg" suppressHydrationWarning>
          Loading...
        </button>
      );
    }
    ...
    if (isCurrent) {
      ...
      return (
        <button className="mt-6 w-full bg-gray-700 text-white py-2 rounded-lg cursor-default" disabled suppressHydrationWarning>
          Current Plan
        </button>
      );
      ...
      return (
        <button
          onClick={() => !freeDisabled && handleUpgrade('free')}
          className={`mt-6 w-full ${freeDisabled ? "bg-gray-500 cursor-not-allowed" : "bg-gray-700 hover:bg-gray-600"} text-white py-2 rounded-lg`}
          disabled={freeDisabled}
          suppressHydrationWarning
        >
          {freeDisabled ? "Already Used" : "Select Plan"}
        </button>
      );
    } else {
      // Check if user is signed in
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="container mx-auto px-4 py-16">
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-[#1e3a5c] via-[#22d3ee] to-[#4ade80] text-transparent bg-clip-text" suppressHydrationWarning>
            Choose Your Plan
          </h1>
